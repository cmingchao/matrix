...
app.directive("user", () => {
    ...
    return {
        template: `<div class="item col col-lg-3 col-sm-3 col-xs-6" >
                    ...
                    <div class="view color-pomo"><a href="/html/userGroup?userType={{item.id}}&r={{type_r}}&f={{type_f}}&m={{type_m}}" target="_blank">查看</a></div>
                </div>`,
        restrict: "E",
        replace: true
    }
...
app.controller("container", function ($scope, $http) {
    "use strict";
    $scope.groupList = [];
    $http.get("../../data-static/userGroup.json").then(
        res => {
            let data = res.data;
    ...
    $scope.screenUser = () => {
        ...
        $.$http_post({
            ...
            callback(res) {
                ...
                $scope.groupList.map((item, index) => {
...
$(function () {
    ...
    let canvasDom2 = document.getElementById("userRFM");//canvas元素节点
    let canvasDom2Width = canvasDom2.offsetWidth;
    let canvasDom2Height = canvasDom2.offsetHeight;
    let arr = [];//存放小几何体
    let x1 = 45, x2 = 55;
    $(function () {
        //RFM图
...
//            设置相机的上方向
        ...
        //设置光源
//            环境光
        let light = new THREE.AmbientLight(0xFFFFFF);//设置环境光(明暗程度相同)，接受一个16进制的颜色值作为参数
        scene.add(light);// 追加光源到场景

        //设置物体
        let axes = new THREE.AxisHelper(100); //坐标轴
        scene.add(axes);

        //箭头
        let arrowHelper = new THREE.ArrowHelper(new THREE.Vector3(250, 0, 0), new THREE.Vector3(-250, 0, 0), 500, 0x666666); //箭头x轴
        ...
        loader.load('../lib/js/FZYaoTi_Regular.js', function (font) {
            ...
            function drawText(x, y, z, text, color = 0x333333) {  //xyz坐标，文字,颜色
                let textGeo = new THREE.TextGeometry(text, text_style); //文字，文字样式
        ...
        //绘制小长方体
        for (let i = 0; i < 8; i++) {
            let boxGeometry = new THREE.BoxGeometry(80, 100, 80);
            let material = new THREE.MeshLambertMaterial({color: 0x5dbdbc, opacity: 0.75, transparent: true});
            let smallCuboid = new THREE.Mesh(boxGeometry, material);
            smallCuboid.userData.uId = i + 1;
            smallCuboid.rotation.x = Math.PI / 2;
            arr.push(smallCuboid);
            scene.add(smallCuboid);
//                边框
            let edges = new THREE.EdgesHelper(smallCuboid, 0x4B9998);//设置边框，可以旋转
            smallCuboid.add(edges);
//                线段
            let lineGeometry = new THREE.Geometry();
            let lineMaterial = new THREE.LineBasicMaterial({color: 0xFAAF66});
            switch (i + 1) {
                case 1:
    ...
    function onDocumentClick(event, u_Id) {
        ...
        if (event.target.tagName === "CANVAS") {
            let mouse = new THREE.Vector3(
                ((event.clientX - event.target.getBoundingClientRect().left) / event.target.offsetWidth) * 2 - 1,
                -((event.clientY - event.target.getBoundingClientRect().top) / event.target.offsetHeight) * 2 + 1,
                0.5);
            let raycaster = new THREE.Raycaster();//新建一个Raycaster对象,获取场景里面的物体,更方便的使用鼠标来操作3D场景。
            raycaster.setFromCamera(mouse, camera);//从相机发射一条射线，经过鼠标点击位置
            let intersects = raycaster.intersectObjects(arr);//计算射线相机到的对象，可能有多个对象，因此返回的是一个数组，按离相机远近排列
        ...
        if (uId) {
            ...
            window.open(`/html/userGroup?userType=${uId}&r=${r}&f=${f}&m=${m}`)
        }
    }

    //刻度
    const scale = (ele, marginL, minW, ...words) => {
        for (let i = 0; i < 6; i++) {
            let div_ele = `<div class="scale" style="left: ${i * 20}%">
...
function myChange(e) {
    let currenVal = +e.value;
    let dataType = $(e).attr("data-type");
    let bList = $(e).siblings(".scale").find("b");
