    ...
    $scope.search = function (isBtnSearch) {
        ...
        $.$http_post({
            ...
            callback(res) {
                ...
                /*数据总条数-即品牌总个数*/
                $scope.brandTotal = data.brandTotal || 0;
                $scope.saleTotal = data.saleTotal || 0;
                /*总销售额*/
                ...
                data.list.map(item => {
                    if (item.c_brand_code) {
                        let retail = item.c_brand_code.substr(0, 2),
                            ...
                            avPrice = item.c_brand_code.substr(5, 1),
                            area = item.c_brand_code.substr(8, 1),
                            retailDetails = $scope.retailDetails[retail];
                        let dt_item = retailDetails.distinction.filter(item => {
                                return item.value == distinct;
                            }),
                            pc_item = $scope.retailList.filter(item => {
                                return item.value == retail;
                            }),
                            area_item = $scope.areaList.filter(item => {
                                return item.value == area;
                            }),
                            avPrice_item = retailDetails["av-price"].filter(item => {
                                return item.value == avPrice;
                            });
                        /*添加品牌区分属性*/
                        ...
                        /*添加区域*/
                        item.area = area_item[0] ? area_item[0].name : "未知";
                        /*添加品牌均价*/
                        item.avPrice = avPrice_item[0] ? avPrice_item[0].name : "未知";
                    } else {
                        /*添加品牌区分属性*/
                        item.distinct = "未知";
                        /*添加业态类型*/
